  ...
  public partial class MainForm : Form
  {
    ...
    private bool RenameFolders
    {
      ...
      set
      {
        ...
        itmOutputCopyTo.ToolTipText   = renameFolders ? "Nicht verf�gbar w�hrend Ordner-Umbenennung" : "Dateien, die �bereinstimmen, werden kopiert und die Kopien umbenannt";
        itmOutputBackupTo.ToolTipText = renameFolders ? "Nicht verf�gbar w�hrend Ordner-Umbenennung" : "Dateien, die �bereinstimmen, werden kopiert und die Originale werden umbenannt";
    ...
    private void OutputMenuItem( object sender )
    {
      ...
      if( clickedMenuItem != itmOutputRenameInPlace )
      {
        ...
        if( clickedMenuItem == itmOutputMoveTo )
          fbdMoveCopy.Description = "W�hrend der Umbenennungsaktion werden Dateien, die mit dem aktuellen regex �bereinstimmen, "
                                  + "in den ausgw�hlten Ordner verschoben und umbenannt (falls n�tig).";
        else if( clickedMenuItem == itmOutputCopyTo )
          fbdMoveCopy.Description = "W�hrend the Umbenennungsaktion werden Dateien, die mit dem aktuellen regex �bereinstimmen, "
                                  + "in den ausgew�hlten Ordner kopiert und ihre Kopien umbenannt (falls n�tig).";
        else if( clickedMenuItem == itmOutputBackupTo )
          fbdMoveCopy.Description = "W�hrend the Umbenennungsaktion werden Dateien, die mit dem aktuellen regex �bereinstimmen, "
                                  + "in den ausgew�hlten Ordner kopiert und die Originale umbenannt (falls n�tig).";
        ...
        if( fbdMoveCopy.SelectedPath == activePath )
        {
          ...
          errorMessage += "' Ordner ist der selbe wie der aktuell ausgew�hlte Ordner.\r\n";
    ...
    private void dgvFiles_CellValidating( object sender, DataGridViewCellValidatingEventArgs e )
    {
      ...
      {
        errorMessage = "Dieser " + strFilename + " beginnt mit einem Leerzeichen oder Punkt. Das ist zwar technisch m�glich, Windows\n"
                     + "l�sst dies aber normalerweise nicht zu, weil es zu Problemen mit anderen Programmen f�hren k�nnte.\n"
                     + "\n"
                     + "Sie Sie sicher, dass Sie fortfahren m�chten?";
    ...
    private void btnRename_Click( object sender, EventArgs e )
    {
      ...
      if( !validMatch ) errorMessage = "Der regul�re Ausdruck in 'Finden' ist nicht g�ltig.";
      ...
      if( errorMessage == null )
      {
        foreach( DataGridViewRow row in dgvFiles.Rows )
        {
          ...
          if( row.Cells[2].Tag != null )
          {
            errorMessage = "Umbennen, w�hrend Fehler existieren, nicht m�glich (rot markiert).";
            break;
          }
        }
      }
      ...
      {
        if( itmOutputMoveTo.Checked ) errorMessage = "'Verschieben nach' Ordner '" + fbdMoveCopy.SelectedPath + "' ist kein g�ltiger Pfad.";
        else if( itmOutputCopyTo.Checked ) errorMessage = "'Kopieren nach' Ordner '" + fbdMoveCopy.SelectedPath + "' ist kein g�ltiger Pfad.";
        else if( itmOutputBackupTo.Checked ) errorMessage = "'Backup nach' Ordner '" + fbdMoveCopy.SelectedPath + "' ist kein g�ltiger Pfad.";
      }
      ...
      {
        if( itmOutputMoveTo.Checked ) errorMessage = "'Verschieben nach' Ordner ist der selbe wie der aktuell ausgew�hlte Ordner.";
        else if( itmOutputCopyTo.Checked ) errorMessage = "'Kopieren nach' Ordner ist der selbe wie der aktuell ausgew�hlte Ordner.";
        else if( itmOutputBackupTo.Checked ) errorMessage = "'Backup nach' Ordner ist der selbe wie der aktuell ausgew�hlte Ordner.";
      }
      ...
      {
        errorMessage = "Einer oder mehrere " + strFilename + " beginnen mit einem Leerzeichen oder Punkt. Das ist zwar technisch m�glich, Windows\n"
                     + "l�sst dies aber normalerweise nicht zu, weil es zu Problemen mit anderen Programmen f�hren k�nnte.\n"
                     + "\n"
                     + "Sie Sie sicher, dass Sie fortfahren m�chten?";
